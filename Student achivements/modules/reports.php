						...
						for($j=$sdate;$j<=$edate;$j=$j+86400)
						{
							...
							$normalized_weekday = strtolower($weekday);
							 if(($normalized_weekday!="saturday") && ($normalized_weekday!="sunday"))
							 {
								 ...
								 $sql = "SELECT sid ,ispresent FROM Achivements WHERE sid=$dsid AND
								...
								{
									echo "<td><a href='index.php?subject=" . $selsub . "&date=" . $currentDate . "'><button type='button' class='btn btn-success btn-sm' style='border-radius:0%'>Take Achivements</button></a></td>";
								}
							}
						}
						if($totlec!=0)
