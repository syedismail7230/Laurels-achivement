	...
	<div class="row text-center">
		<div class="col-md-12 col-lg-12">
			...
			<div class="margin-top-bottom-medium">
				<button type="submit" class="btn btn-success btn-block" style='border-radius:0%;' name="sbt_top"><i class="glyphicon glyphicon-ok-sign"></i> Save Achivements</button>
			</div>
			...
			<table class="table table-striped table-hover">
				<thead>
					<tr>
						...
						<th class="text-center"><input type="checkbox" class="chk-head" /> All Achived</th>
					</tr>
				</thead>
				...
				<?php
					 ...
					 $ddate = strtotime($dat);
					 $sub=$_GET['subject'];
					$que= "SELECT sid, aid, ispresent  from Achivements  WHERE date  =$ddate
			...
			<div class="alert alert-dismissible alert-danger">
				...
				<strong>Sorry!</strong> Achivements cannot be recorded for future dates!.
			</div>	
			...
			<?php
				...
				if (isset($_POST['sbt_top'])) {
					if(isset($_POST['updateData']) && ($_POST['updateData'] == 1) ) {
						...
						// prepare sql and bind parameters
							...
							{
									//echo "hii";
								// UPDATE `Achivements` SET `ispresent` = '1' WHERE `Achivements`.`aid` = 79;

									$stmtInsert = $conn->prepare("UPDATE Achivements SET ispresent = :isMarked WHERE aid = :aid"); 
                ...
                <button type="button" class="close" data-dismiss="alert">×</button>
                <strong>Well done!</strong> Achivements Recorded Successfully!.
              </div>';	
						...
						$tstamp = strtotime($date);
							...
							{
									//echo "hii";
									$stmtInsert = $conn->prepare("INSERT INTO Achivements (sid, date, ispresent, uid, id) 
